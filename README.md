...
https://login.salesforce.com/packaging/installPackage.apexp?p0=04to0000000EFQ0

Note: If you are installing into a sandbox organization use the following url instead:
http://test.salesforce.com/packaging/installPackage.apexp?p0=04to0000000EFQ0
...
* Volunteer Pref Fields
  * `Volunteer Preferences Per Org` - contains volunteer preferences on a per org basis (global preferences are maintained in the Contact object). To add new objects edit the contact layout. Look under related lists for "Volunteer Prefere..." and add it to the layout.
  * `Campaign Member.Sponsoring Org Email Opt Out` - this field is automatically maintained for every campaign a volunteer is a member of. It reflects if the user has opted out of emails from the org associated with this campaign. Campaigns not associated with any org are automatically associated with the root organization as specified in the Karma Exchange admin settings. Note that both `Campaign Member.Sponsoring Org Email Opt Out` and `Contact.Email Opt Out` should be checked when deciding whether or not to send emails to volunteers. `Contact.Email Opt Out` is a global preference whereas `Campaign Member.Sponsoring Org Email Opt Out` is a per org preference.
